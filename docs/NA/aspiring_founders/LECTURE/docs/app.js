...
var openDashboard = window.openDashboard = function() {
  ...
  if (toggleBtn) {
    ...
    toggleBtn.innerHTML = '<svg style="width:16px;height:16px;fill:currentColor" viewBox="0 0 24 24"><path d="M21,16V4H3V16H21M21,2H3C1.89,2 1,2.89 1,4V16C1,17.1 1.89,18 3,18H10V20H8V22H16V20H14V18H21C22.1,18 23,17.1 23,16V4C23,2.89 22.1,2 21,2Z"></path></svg> 슬라이드보기 <span class="badge">Esc</span>';
  }
  renderDashboard();
};
...
var closeDashboard = window.closeDashboard = function() {
  ...
  if (toggleBtn) {
    ...
    toggleBtn.innerHTML = '<svg style="width:16px;height:16px;fill:currentColor" viewBox="0 0 24 24"><path d="M4,4H20V12H4V4M4,14H11V20H4V14M13,14H20V20H13V14Z"></path></svg> 대시보드 <span class="badge">Esc</span>';
  }
};
...
var renderDashboard = window.renderDashboard = function() {
  ...
  allSlides.forEach(function(slide, idx) {
    ...
    var title = titleEl ? titleEl.textContent.trim() : '슬라이드 ' + (idx + 1);
    var category = categoryEl ? categoryEl.textContent.trim() : '강의 슬라이드';
  ...
  if (matchCount === 0) {
    cardsGrid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:#94a3b8;font-size:1.1rem;">🔍 검색 조건에 맞는 슬라이드가 없습니다.</div>';
  }
};
  ...
  if (timerRunning) {
    ...
    timerInterval = setInterval(function() {
      ...
      if (timerRemaining <= 0) {
        ...
        if (btn2) btn2.textContent = '타이머 시작';
      }
    }, 1000);
  }
};
...
var toggleDrawing = window.toggleDrawing = function() {
  ...
  if (btn) {
    ...
    if (span) span.textContent = drawingEnabled ? '포인터 펜 끄기 (P)' : '포인터 펜 켜기 (P)';
...
var toggleTheme = window.toggleTheme = function() {
  ...
  if (isLight) {
    ...
    if (btn) { var s = btn.querySelector('.btn-text'); if (s) s.textContent = '발표용 라이트 모드'; }
  } else {
    root.setAttribute('data-theme', 'light');
    if (btn) { var s2 = btn.querySelector('.btn-text'); if (s2) s2.textContent = '발표용 다크 모드'; }
  }
};
